 ...
 * 7 kyu: uniquePush
 *
 * Write a function that will take an array and a person object as parameters. The function will only push a "person" object 
 * onto the end of an array if someone with that phone number doesn't already exist in that array.
...
function uniquePush(arr, obj) {
  for (var i = 0; i < arr.length; i++) {
    if (arr[i].phoneNumber !== obj.phoneNumber) {
      arr.push(obj);
      return true;
    } else if (arr[i].phoneNumber === obj.phoneNumber) {
      return false;
