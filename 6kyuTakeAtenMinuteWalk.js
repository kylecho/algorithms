...
function isValidWalk (walk) {
  ...
  nLength = (walk.join('').toString().match(/n/gi)) ? 
    walk.join('').toString().match(/n/gi).length
    : 0;
  
  sLength = (walk.join('').toString().match(/s/gi)) ?
    walk.join('').toString().match(/s/gi).length
    : 0;
    
  wLength = (walk.join('').toString().match(/w/gi)) ?
    walk.join('').toString().match(/w/gi).length
    : 0;
    
  eLength = (walk.join('').toString().match(/e/gi)) ?
    walk.join('').toString().match(/e/gi).length
    : 0;
  
  if (walk.length !== 10) return false;
  if (eLength !== wLength) return false;
  if (sLength !== nLength) return false;
